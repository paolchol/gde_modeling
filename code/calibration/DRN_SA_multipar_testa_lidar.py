...
drn_data = os.path.join(cwd,'busca_drain_specifiche_celle_lidar.csv') # DRN characteristics
model_ws = cwd
model_name = 'busca_base_infittito_apr24_drain_testa_lidar'
...
drn = load_drn_file(os.path.join(model_ws, 'busca_base_infittito_apr24_drain_testa_lidar_start.drn'))
...
k_dict = {
    'kt' : np.linspace(0.00001, 0.0001, num = 100), 
    'ka': [0.00001]
}
...
df_results.to_excel(os.path.join(model_ws, 'drain_results_multipar_lidar.xlsx'))
drain_outflow_save.to_csv(os.path.join(model_ws, 'drain_outflow_multipar_lidar.csv'))
# %%
