...
cwd = '\\'.join(os.getcwd().split('\\')[:-2])   #where test files are stored (github)
drn_data = os.path.join(cwd, 'input_data', 'Busca_DRN_testa_1','busca_drain_specifiche_celle_lidar.csv') # DRN characteristics
model_ws = os.path.join(cwd, 'models', 'Busca_DRN_luglio')
model_name = 'busca_base_infittito_luglio_DRN'
...
drn = load_drn_file(os.path.join(cwd, 'input_data', 'Busca_DRN_testa_1', 'busca_base_infittito_apr24_drain_testa_lidar_start.drn'))
...
for kt in k_dict['kt']:
    ...
    for ka in k_dict['ka']:
        ...
        stress_period_data = {0: drn.iloc[:, :].to_numpy().tolist()}
        ...
        outputs.append([f'M{m}', kt, ka, sum_drain_t, sum_drain, sum_drain_tot])
        m += 1
...
df_results = pd.DataFrame(outputs, columns = ['model', 'kt', 'ka', 'drn_head', 'drn_until_rc', 'drn_total'])
df_results.to_excel(os.path.join(cwd, 'output_data', 'validation', 'C1_results_july24.xlsx'))
drain_outflow_save.to_csv(os.path.join(cwd, 'output_data', 'validation', 'C1_outflow_july24.csv'))
